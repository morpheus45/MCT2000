...
import { Calendar, MapPin, Users, Check, Loader2, Map as MapIcon, ChevronDown, ChevronUp } from "lucide-react";
import { getSupabaseBrowser, supabaseConfigured } from "@/lib/supabase/client";
import { isDemo, demoMe, demoEvents } from "@/lib/demo";
import { cn } from "@/lib/utils";

// Lazy-load the map only when needed
const ClubMap = dynamic(() => import("./ClubMap"), { ssr: false, loading: () => null });
    ...
    <section className="mx-auto max-w-5xl px-5 py-16">
      ...
      <div className="space-y-4">
        {events.map((e, i) => {
          ...
          const hasMap = Array.isArray(e.waypoints) && e.waypoints.length > 1;
          const mapOpen = openMap === e.id;
          ...
          return (
            <motion.article
              ...
              className="overflow-hidden rounded-2xl border border-white/10 bg-ink-900/60 transition-all hover:border-flame-500/40"
            >
              {/* Main row */}
              <div className="grid items-stretch gap-0 md:grid-cols-[200px_1fr_auto]">
                ...
                <div className="p-5">
                  ...
                  {hasMap && (
                    <button
                      onClick={() => setOpenMap(mapOpen ? null : e.id)}
                      className="mt-3 flex items-center gap-1.5 text-xs text-flame-400/80 hover:text-flame-400 transition-colors"
                    >
                      <MapIcon className="h-3.5 w-3.5" />
                      {mapOpen ? (
                        <>Masquer l'itinéraire <ChevronUp className="h-3 w-3" /></>
                      ) : (
                        <>Voir l'itinéraire <ChevronDown className="h-3 w-3" /></>
                      )}
                    </button>
                  )}
                </div>
              ...
              {hasMap && mapOpen && (
                <div className="border-t border-white/10 p-4">
                  <ClubMap
                    customWaypoints={e.waypoints ?? undefined}
                    customLabel={e.title}
                    heightClass="h-64 w-full overflow-hidden rounded-xl border border-white/10 bg-ink-900/60"
                  />
                </div>
              )}
            </motion.article>
